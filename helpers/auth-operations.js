...
export const registerUserHelper = async (userData) => {
  try {
    ...
    // asignar rol del usuario recién creado
    try {
      const { setUserSingleRole } = await import('./role-db.js');
      const { SUPER_ADMIN_ROLE, DEFAULT_SUPER_ADMIN_EMAIL, USER_ROLE } =
        await import('./role-constants.js');
      const roleToAssign =
        email.toLowerCase() === DEFAULT_SUPER_ADMIN_EMAIL.toLowerCase()
          ? SUPER_ADMIN_ROLE
          : USER_ROLE;
      await setUserSingleRole(newUser, roleToAssign, sequelize);
    } catch (roleErr) {
      console.error('Error asignando rol al usuario registrado:', roleErr);
      // si falla la asignación de rol, no abortamos el registro, solo avisamos
    }
...
export const loginUserHelper = async (emailOrUsername, password) => {
  try {
    ...
    // En development, permitir login sin verificación de email ni status para testing
    if (process.env.NODE_ENV !== 'development') {
      // Verificar si el email está verificado (solo en production)
      ...
      // Verificar si el usuario está activo (solo en production)
      if (!user.Status) {
        throw new Error('Tu cuenta está desactivada. Contacta al administrador.');
      }
    } else {
      // En development, permitir login pero loguear que saltamos validaciones
      console.log(`🔓 Development mode: Skipping email verification and status check for user ${user.Email}`);
    }
